...
当前 `apps/core-api/` 除安全、数据与模块化地基外，已完成 G1“需求草案 -> 人工确认 -> 招聘任务”、G2“岗位方案 -> 版本修改 -> 人工批准”、独立简历库“真实文件 -> 不可变版本 -> 解析证据 -> 候选规范化”、G3“ResumeVersion -> 硬条件过滤 -> 固定评分与证据”和企业知识生命周期。简历库支持 PDF、DOC、DOCX、TXT，服务端计算 SHA-256，以 Apache Tika 提取正文并用确定性规则生成姓名、联系方式、学历、年限、技能和地点字段及原文偏移证据；前端简历库与 G3 已接入。实现包含幂等、乐观锁、租户隔离、敏感字段加密、确认哈希、AgentRun、MatchRun、逐项 ResumeVersion 证据和审计；解析失败同样保存不可变文件版本、失败码与 retryable，不生成虚构候选事实。G2 可固化已发布知识版本引用，但 G2/G3 当前均为确定性引擎，不调用 LLM 或 RAG 内容检索。名单确认、推荐报告、向量知识检索、真实身份/权限和 PostgreSQL 生产实例仍未实现，因此尚不构成完整生产招聘智能体；ATS 与在线面试当前仅保留接口契约。
