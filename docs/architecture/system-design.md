...
知聘是一套以 ATS 嵌入插件为生产主形态、同时保留独立管理和降级入口的受控招聘智能体平台。系统围绕招聘任务，完成岗位方案生成、人才检索与匹配、面试协同、综合评价、知识维护和运行审计。
...
1. **基础可运行**：前端 TypeScript 化、Embed SDK 与宿主测试壳、OpenAPI 契约、Java 核心骨架、认证、多租户、任务和审计。
2. **知识与匹配闭环**：文档解析、索引、岗位方案、评分卡、混合检索、证据提取和人工名单确认。
3. **外部协同闭环**：接入一个 ATS、一个面试工具和一个消息渠道，完成重试与人工补偿。
4. **生产试点**：历史回测、安全评估、性能测试、客户 UAT、监控与运维手册。
