...
当前 `apps/core-api/` 除安全、数据与模块化地基外，已完成 G1“需求草案 -> 人工确认 -> 招聘任务”、G2“岗位方案 -> 版本修改 -> 人工批准”、G3“候选输入 -> 标准化简历版本 -> 硬条件过滤 -> 固定评分与证据”和企业知识生命周期。实现包含幂等、乐观锁、租户隔离、敏感字段加密、确认哈希、AgentRun、MatchRun、逐项 ResumeVersion 证据，以及知识文件校验、不可变版本、解析状态、审核发布、停用和归档恢复；前端已接入这些服务。G2 可固化已发布知识版本引用，但 G2/G3 当前均为确定性引擎，不调用 LLM 或 RAG 内容检索。名单确认、推荐报告、向量知识检索、真实身份/权限和 PostgreSQL 生产实例仍未实现，因此尚不构成完整生产招聘智能体；ATS 与在线面试当前仅保留接口契约。
