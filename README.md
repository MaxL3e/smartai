...
- 当前实现：G1-G4、企业知识生命周期和独立简历库已接入本地 Core API；真实简历文件上传与 G3 匹配是主入口，12 位虚构样本仅保留为显式次级演示入口；G4 会先展示冻结预览，再经人工门禁生成不可变名单和版本化推荐报告；在线面试为接口预留
...
4. 选择候选人，核对同屏展示的原文证据、系统判定、待核实项和外部影响，再由招聘负责人确认并冻结名单；查看或下载服务端版本化推荐报告。
...
当前前后端已实现知识资料条目、文件 SHA-256 校验、上传会话、不可变版本、文本解析与索引、人工审核发布、停用、归档恢复、租户隔离和审计。文本、Markdown 与 JSON 可进入解析和索引；简历库的 PDF、DOC、DOCX 与 TXT 由 Apache Tika 真实解析，知识库的通用 Office/PDF 解析仍需继续补齐。向量检索、语义召回和 RAG 内容生成仍未实现。

## 当前实现范围

已实现：

- G1-G4 可点击业务链路、服务端名单确认、推荐报告和页面状态联动
- 对话式招聘任务创建与岗位方案生成体验
- 可编辑的岗位评分卡、权重和推荐阈值
- 候选人级匹配证据、简历查看和名单选择
...
- 名单确认 G4 后端闭环：短期冻结预览、输入哈希、人工 checkpoint、乐观锁与幂等、不可变候选名单、任务阶段推进、审计和版本化推荐报告；报告可按 TXT 或 JSON 下载
...
- 后续招聘业务 endpoint：向量知识检索、面试编排、结果回收和综合评价
...
`apps/core-api/` 已实现招聘智能体 G1-G4、企业知识生命周期和独立简历库：G1 负责将自然语言需求整理为可确认草案并创建招聘任务；G2 负责生成、修改和人工批准岗位方案与评分卡，并固化已发布知识版本引用；独立简历库负责 PDF、DOC、DOCX、TXT 原件上传、SHA-256、不可变文件版本、Tika 解析、字段证据和候选规范化；G3 使用解析成功的 ResumeVersion 执行硬条件过滤、确定性加权评分和逐项证据定位；G4 冻结待确认正文，经人工 checkpoint 固化候选名单并生成与任务、方案、评分卡、匹配运行和简历证据绑定的推荐报告；知识服务负责资料条目、文件上传、版本、解析状态、审核发布、停用和归档恢复。前端已接入这些接口；服务异常会明确显示错误，不会把固定数据冒充服务结果。G2 使用 `DETERMINISTIC_DEMO`，G3 使用 `DETERMINISTIC_RULES`，两者均未调用真实 LLM 或 RAG 内容检索。
